...
class WalletSerializer(serializers.ModelSerializer):
    class Meta:
        model = Wallet
        fields = ['wallet_id', 'merchant', 'amount',
    ...
    def validate(self, data):
        ...
        if request and not request.user.is_superuser:
            raise serializers.ValidationError(
                "Only superusers can modify wallet details.")
        return data
...
class WithdrawalSerializer(serializers.ModelSerializer):
    class Meta:
        model = Withdrawal
        fields = ['sn', 'withdrawal_id', 'merchant', 'amount',
                  'initial_balance', 'final_balance', 'status', 'createdAt', 'updatedAt']
