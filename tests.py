"""
This module contains unittest for Authentication and Wallets
"""
import unittest
...
class TestVerificationFunctions(unittest.TestCase):
    """Verification Function to generate email verification token
    Args:
        unittest (_type_):
    """
    ...
    def test_store_verification_token(self, mock_config, mock_redis):
        ...
        result = utils.store_verification_token('test@example.com', 'token123')
    ...
    def test_get_merchant_data(self, mock_redis):
        ...
        result = utils.get_merchant_data('test@example.com')
        self.assertEqual(result, {'name': 'Merchant'})

    @patch('authentication.utils.redis_client')
    def test_clear_merchant_data(self, mock_redis):
        mock_redis.delete.return_value = 1
        result = utils.clear_merchant_data('test@example.com')
        self.assertTrue(result)
    ...
    def test_verify_token(self, mock_redis):
        ...
        result = utils.verify_token('test@example.com', 'token123')
        self.assertTrue(result)
    ...
    def test_send_verification_email(self, mock_config, mock_send_mail):
        ...
        result = utils.send_verification_email('test@example.com', 'token123')
        self.assertTrue(result)
...
class EmailVerificationTests(TestCase):
    def setUp(self):
        ...
        self.verify_email_url = reverse('merchant-verify-email')
    ...
    def test_successful_verification(self, mock_clear_data, mock_get_data, mock_verify):
        ...
        self.assertEqual(merchant.business_name,
                         self.test_merchant_data['business_name'])
