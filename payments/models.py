...
class PaymentGateway(models.Model):
    ...
    gateway_logo = models.ImageField(
        upload_to="gateway_logo/", null=True, blank=True)
...
class MerchantPaymentGateway(models.Model):
    ...
    """
    sn = models.IntegerField(primary_key=True, unique=True, db_index=True,
                             verbose_name="Serial Number")  # Matches Sequelize's `autoIncrement: true`
    merchant = models.ForeignKey(
        Merchant, on_delete=models.CASCADE, related_name='merchant_payment_gateway')
    # Store JSON data for active payment gateways
    payment_gateways = models.JSONField(blank=False)
    createdAt = models.DateTimeField(auto_now_add=True)
    updatedAt = models.DateTimeField(auto_now=True)
    ...
    def save(self, *args, **kwargs):
        if not self.sn:  # Only assign if 'sn' is empty
            last_sn = MerchantPaymentGateway.objects.exclude(
                sn='').order_by('-sn').first()
            self.sn = last_sn.sn + 1 if last_sn else 1
        super().save(*args, **kwargs)
